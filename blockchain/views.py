"""
Learning material to understand blockchain.
# https://blog.coincodecap.com/what-is-blockchain-a-simple-guide-for-dummies
"""
...
class Blockchain:
    ...
    def create_block(self, nonce, previous_hash):
        ...
        block = {
            ...
            'nonce': nonce, # The nonce is a random, one-time, whole number[32-bit].
            'transactions': self.transactions,
            'timestamp': str(datetime.datetime.now()), #  time at which the block was added to the network, included by hashing in block.
            'previous_hash': previous_hash,
            # blockchain is an immutable ledger,you can’t change the block information once it’s hashed and linked to the next block
        }
        self.transactions = []
        self.chain.append(block) # adding newly generated block to the chain.
        return block
    ...
    #  the network difficulty. If a miner finds such a nonce, called a golden nonce, then they win the right to add that
    #  block to the blockchain and receive the block reward
    def hash(self, block):
